            ...
            <div class="search-container mt-4">
                ...
                @if(request('search'))
                    <div class="search-results-info animate-on-scroll">
                        ...
                        <a href="{{ route('courses.index') }}" class="clear-search-btn">
    ...
    <div class="container">
        ...
        <div class="section-header animate-on-scroll">
            @if(request('search'))
                ...
                <h2>Found {{ $courses->total() }} {{ Str::plural('Course', $courses->total()) }}</h2>
                <p>Here are the courses matching your search criteria</p>
            @else
                <div class="section-badge">Course Catalog</div>
                <h2>Browse All Courses</h2>
                <p>Explore our diverse collection of educational courses</p>
            @endif
        </div>
        ...
        <div class="notes-grid">
            @forelse($courses as $index => $course)
                <div class="benefit-card animate-on-scroll" style="--delay: {{ $index * 0.1 }}s">
                    ...
                    <div class="benefit-content">
                        <h3>{{ $course->name }}</h3>
                        <p class="note-preview">{{ Str::limit($course->description, 150) }}</p>
                        <div class="note-meta">
                            <div class="meta-item">
                    ...
                    <div class="benefit-actions">
                        <form method="POST" action="{{ route('courses.enroll', $course->id) }}">
                            @csrf
                            <button type="submit" class="action-btn enroll-btn">
                                <i class="bi bi-person-plus me-2"></i>
                                <span>Enroll</span>
                            </button>
                        </form>
                        <a href="{{ route('courses.show', $course->id) }}" class="action-btn read-btn">
            ...
            @empty
                <div class="empty-state animate-on-scroll">
                    ...
                    @if(request('search'))
                        <p>We couldn't find any courses matching "<strong>{{ request('search') }}"</strong>.</p>
        ...
        @if($courses->hasPages())
            <div class="pagination-wrapper animate-on-scroll">
                <div class="pagination-info">
                    <span>Showing {{ $courses->firstItem() }} to {{ $courses->lastItem() }} of {{ $courses->total() }} results</span>
                </div>
